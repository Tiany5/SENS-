...
#' @export 
SENS_SSMT <- function(t,t0,alpha,option=c('Gaussian','General')){
  m=length(t)
  if (option=='Gaussian'){
...
SENS_SSMT_Oracle <- function(t,t0,alpha){
  m=length(t)
  sens.numerator <- dnorm(c(t,t0),0,1)
  sens.denominator <- (1-0.1)*dnorm(c(t,t0),0,1)+0.1*dnorm(c(t,t0),0.4*log(m),1);
  sens.Est <- sens.numerator[1:m]/sens.denominator[1:m];
  sens.Est0 <- sens.numerator[(m+1):(2*m)]/sens.denominator[(m+1):(2*m)];
  t<-sapply(1:m, function(i) sign(sens.Est0[i]-sens.Est[i])*(max(exp(-sens.Est[i]),exp(-sens.Est0[i]))))
  y = bc.func(t,alpha)
